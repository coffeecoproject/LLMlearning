...
> 基础 Agent 可以直接使用当前用户请求；本篇只解释系统主动采用正式 Goal 形成流程时怎样控制权威边界。
...
只学习通用 Agent 框架时，读完本组即可进入 [[00-Agent循环与生命周期概览|Agent 循环与生命周期概览]]。如果准备构建可审计的独立需求入口，再选读 [[00-Goal-Intake高可靠扩展概览|Goal Intake 高可靠扩展概览]]。
