...
- Keep product interaction objects separate from execution objects: in current Codex terminology, a Thread contains user Turns and a Turn runs from user input to the final agent message; one Turn can contain multiple Model Calls, tool executions, and Items. Do not redefine Turn as one model request/response. Treat Item as a protocol/recording unit and Observation as the semantic role of environment feedback.
...
- Organize the Agent required-reading mainline around the causal execution path: user request and Goal → create or resume Run and state → build context → call the model → parse a decision → optionally plan → execute an authorized tool → record Observation and artifacts → update state or recover → verify and accept. Treat persistence, permissions, observability, and cost as cross-cutting responsibilities even when their dedicated notes appear after the main execution path.
...
- Treat `Goal Intake` as an optional high-reliability product or orchestration pattern, not an industry-standard Agent component. Keep the general problem of goal formation separate from a named Intake implementation. Do not attribute an independent Goal Intake stage to CodeClosure unless its current repository documents or implements one; distinguish the current explicit `CreateGoal` boundary from possible future natural-language intake.
