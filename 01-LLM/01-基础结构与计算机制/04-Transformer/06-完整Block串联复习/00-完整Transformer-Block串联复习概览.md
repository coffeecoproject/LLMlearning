...
next: "[[01-一次Block完整数据流|一次Block完整数据流]]"
...
1. [[01-一次Block完整数据流|一次 Block 完整数据流]]：沿一个常见 Pre-Norm Block 逐步走完；
2. [[02-完整Block形状追踪|完整 Block 形状追踪]]：用 `[1,3,4]` 主干和 `intermediate_size=8` 检查形状；
3. [[03-Transformer基础结构最终检查|Transformer 基础结构最终检查]]：检查是否能独立解释整个 Transformer。
...
下一篇：[[01-一次Block完整数据流|一次 Block 完整数据流]]。
