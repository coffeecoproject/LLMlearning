...
previous: "[[Head拼接与Output-Projection]]"
next: "[[MHA-GQA与MQA概览]]"
...
位置机制不一定都在这一步加入。例如，[[RoPE的作用位置与直觉|RoPE]] 通常先改变 Q、K，再由它们计算 Score；[[ALiBi的作用位置与直觉|ALiBi]] 则把与相对距离有关的偏置加入 Score。这里不把不同位置方案强行画成同一条计算步骤，具体路径返回[[Position位置机制概览|Position 位置机制]]复习。
...
返回：[[Multi-Head-Attention概览|Multi-Head Attention]]。下一专题：[[MHA-GQA与MQA概览|MHA、GQA 与 MQA]]。
