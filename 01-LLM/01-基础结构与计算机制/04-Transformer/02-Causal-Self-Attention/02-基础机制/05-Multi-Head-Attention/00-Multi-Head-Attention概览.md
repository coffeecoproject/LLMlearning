...
previous: "[[00-Value加权与Context-Mixing概览|Value加权与Context-Mixing概览]]"
...
1. [[01-Attention-Head是什么|Attention-Head是什么]]：明确一个 Head 到底包含什么。
2. [[02-为什么需要多个Head|为什么需要多个Head]]：理解单套 Weight 的限制和并行表示的价值。
3. [[03-QKV怎样组织成多个Head|QKV怎样组织成多个Head]]：看懂 Projection、Reshape 和 Head 维度。
4. [[04-每个Head怎样独立产生Context|每个Head怎样独立产生Context]]：把单头 Attention 流程复制到多条并行路径。
5. [[05-Head拼接与Output-Projection|Head拼接与Output-Projection]]：理解多个结果怎样回到模型主表示空间。
6. [[06-Multi-Head-Attention完整流程|Multi-Head-Attention完整流程]]：用一组简单形状串起整个数据流。
...
本专题先建立**标准 MHA**心智模型。MHA、GQA、MQA 怎样共享 K/V Head，进入[[00-MHA-GQA与MQA概览|下一专题]]；MLA 怎样改变 K/V 表示与缓存路径，进入[[00-MLA与注意力变体概览|扩展专题]]。
...
下一专题：[[00-MHA-GQA与MQA概览|MHA、GQA 与 MQA]]。
