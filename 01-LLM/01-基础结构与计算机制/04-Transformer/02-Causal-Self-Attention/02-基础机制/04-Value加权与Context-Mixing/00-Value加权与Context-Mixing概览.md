...
previous: "[[00-从匹配强弱到信息权重概览|从匹配强弱到信息权重概览]]"
next: "[[00-Multi-Head-Attention概览|Multi-Head-Attention概览]]"
...
它没有携带要传递的完整内容，也还不是更新后的 Token 表示。接下来必须把 Weight 与 [[04-Value是什么|Value]] 配对：
...
1. [[01-为什么Weight要作用于Value|为什么 Weight 要作用于 Value]]：分清“选择依据”和“传递内容”。
2. [[02-Value加权求和怎样计算|Value 加权求和怎样计算]]：用小向量走完缩放与逐维相加。
3. [[03-Context-Vector是什么|Context Vector 是什么]]：明确这一步产生了什么、还没有产生什么。
4. [[04-为什么每个位置得到不同的Context|为什么每个位置得到不同的 Context]]：从不同 Query、不同可见范围理解逐位置混合。
...
- [[00-Multi-Head-Attention概览|Multi-Head Attention]]；
- [[00-Residual与Normalization概览|Residual 与 Normalization]]；
- [[00-Output-Layer输出层概览|Output Layer]]。
...
下一节：[[00-Multi-Head-Attention概览|Multi-Head Attention]]。
