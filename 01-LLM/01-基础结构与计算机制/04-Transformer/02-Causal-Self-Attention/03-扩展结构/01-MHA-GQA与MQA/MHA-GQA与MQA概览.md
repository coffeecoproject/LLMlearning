...
1. [[为什么要区分Query-Head与KV-Head]]：先拆开以前被统称为 Head 的两类数量。
2. [[MHA怎样一一对应Q与KV-Head]]：用标准 MHA 建立一一对应基线。
3. [[MQA怎样让所有Query-Head共享KV]]：理解多个 Query 怎样共用一组 K/V。
4. [[GQA怎样让Query-Head分组共享KV]]：理解介于两端之间的分组方式。
5. [[MHA-GQA与MQA完整对比]]：用同一组数字和形状横向比较。
6. [[怎样从模型配置判断Head结构]]：读取真实配置字段，并观察 Qwen3 与 OpenAI `gpt-oss`。
...
这些结论来自对应版本公开配置，完整证据与 `hidden_size` 边界放在[[怎样从模型配置判断Head结构|配置阅读笔记]]。
...
下一专题：[[MLA与注意力变体概览|MLA 与注意力变体（扩展）]]。
