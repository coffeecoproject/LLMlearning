...
1. [[MLA为什么需要压缩KV表示]]：先理解它要解决的结构问题。
2. [[低秩联合压缩怎样表示K与V]]：理解“低秩”“联合”和“潜在表示”分别指什么。
3. [[为什么MLA要解耦RoPE]]：理解位置信息为什么要使用一条单独路径。
4. [[MLA完整流程]]：把 Query、KV Latent、RoPE、Score、Context 串成一条因果链。
5. [[MLA与MHA-GQA-MQA有什么不同]]：分清“减少 KV Head”和“压缩 KV 表示”。
6. [[怎样阅读DeepSeek-V3的MLA配置]]：读取真实模型的维度和官方实现。
7. [[其他注意力变体地图]]：按“究竟改了 Attention 的哪一部分”认识其他常见名称。
