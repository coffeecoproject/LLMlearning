...
1. [[1.2.1-向量基础概览|1.2.1 向量基础]]：只学习理解 Embedding 所需的向量、维度、形状与相似性直觉。
2. [[1.2.2-Token-Embedding概览|1.2.2 Token Embedding]]：理解 Embedding Matrix 和 Lookup。
3. [[1.2.3-Embedding参数与语义边界概览|1.2.3 Embedding 参数与语义边界]]：认识 Embedding 是模型参数，但不等于完整词义。
4. [[1.2.4-初始表示与Hidden-State概览|1.2.4 初始表示与 Hidden State]]：区分初始向量与经过上下文处理的动态表示。
...
- Position 不再混在 Embedding 中，独立进入 [[1.3-Position位置机制概览]]。
- Attention、FFN、Residual 与 Norm 进入 [[1.4-Transformer概览]]。
...
下一专题：[[1.3-Position位置机制概览|Position 位置机制]]。
