...
## 这一步是在 Output Layer 中完成吗

不完全是。一轮 Decode 横跨整条模型路径：

| 环节 | 这一轮负责什么 |
|---|---|
| Embedding | 把刚选出的 Token ID 变成初始向量 |
| Transformer Blocks | 逐层处理当前 Token；其中 Attention 读取历史 KV Cache，并加入当前 Token 的新 K、V |
| Output Layer | 只把最终 Hidden State 转换为下一轮 Logits |
| 生成控制器 | 在模型外处理 Logits，并选出下一个 Token |

所以 Output Layer 是每轮增量 Decode 的最后一段模型计算，不是整个 Decode；KV Cache 是 Attention 使用的历史状态，也不是一个独立处理 Token 的模型层。
