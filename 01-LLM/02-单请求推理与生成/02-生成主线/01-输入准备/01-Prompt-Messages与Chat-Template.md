...
## 在真实应用中，Messages 从哪里来

本篇从“一组 Messages 已经准备好”开始解释 Chat Template。但在 Codex 这样的真实 Agent 中，Messages 之前还有一段上游过程：
...
→ 加入指令、环境和工具结果
→ 得到本次模型可见 Messages
→ Chat Template / 模型输入协议
→ Tokenizer
```

完整源码案例见：[[08-从Codex线程到模型输入的完整案例|从 Codex 线程到模型输入的完整案例]]。
